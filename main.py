...
class Runner:
    ...
    def run(self):
        ...
        self.stage = 0
        print("self.args.alpha_",self.args.alpha_)
        print("self.args.strategy",self.args.strategy)
        ...
        with tqdm(total=self.args.epoch + self.args.epoch_per_CYCLES * self.args.CYCLES) as _tqdm:
            for cycle in range(self.args.CYCLES + 1):
                if cycle != 0:
                    ...
                    self.logger.info(
                        f"Active learning: {self.args.CYCLES} cycles × {self.args.epoch_per_CYCLES} epochs. ")
                    self.logger.info(
                        f"Total: {self.args.epoch + self.args.epoch_per_CYCLES * self.args.CYCLES} epochs.")
        ...
        # torch.save(self.model.state_dict(), "FBDB15K_RQ4.pth")
    ...
    def compute_lambda_l21(self, D, strategy='frobenius', factor=0.3):

        frobenius_norm = torch.norm(D, p='fro')

        row_l2_norms = torch.norm(D, p=2, dim=1)

        if strategy == 'frobenius':
            lambda_value = frobenius_norm.item() / D.size(0) ** 0.5 * factor
        elif strategy == 'mean':
            lambda_value = torch.mean(row_l2_norms).item() / D.size(0) ** 0.5 * factor
        elif strategy == 'max':
            lambda_value = torch.max(row_l2_norms).item() / D.size(0) ** 0.5 * factor
    ...
    def active_learning_sampling(self, _tqdm, sub_embs, cycle):
        ...
        with torch.no_grad():
            ...
            #     'final_emb': final_emb,
            #     'left_ents': left_ents,
            #     'right_ents': right_ents,
            #     'min_distances': min_distances,
            #     'x_index': x_index,
            #     'y_index': y_index
            # }
            #
            # with open('FBDB_al_data.pkl', 'wb') as f:
            ...
            lambda_frobenius = self.compute_lambda_l21(C_diversity_score, self.args.strategy, self.args.alpha_)

            Z_C = optimize_ds3_regularized(A_matrix_xy_pair, lambda_frobenius, self.args.rho, C_diversity_score,
                                           max_iteration,
    ...
    def _test(self, test_left, test_right, last_epoch=False, save_name="", loss=None):
        ...
        if acc_r2l[0] > self.best_hit_1:
            self.best_hit_1 = acc_r2l[0]
        ...
        #     self.logger.info(
        #         f"Best model update in Ep {self.epoch}: MRR from [{max(self.loss_log.acc)}] --> [{mrr_l2r}] ... ")
